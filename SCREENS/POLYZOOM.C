...
void CybervectorEntry (FSM* _fsm)
{
	...
	u16 polysDataTempSize = (u16) LOADgetEntrySize ( &RSC_DISK1, RSC_DISK1_POLYZOOM__CYBERVECTOR_BIN);
	...
	
	{
		...
		for ( i = 0 ; i < ARRAYSIZE(fileoffsets) ; i++ )
		{
			...
			u16 offset = (u16) RSC_DISK1.metaData [ rscOffsetId ].offset;
			u16 size   = (u16) RSC_DISK1.metaData [ rscOffsetId ].size;
	...
	pald = (u16*)(temp + RSC_DISK1.metaData [RSC_DISK1_METADATA_POLYZOOM_PAL1_PAL].offset);
...
void CybervectorBacktask (FSM* _fsm)
{
	...
	u16		i, x, y, size;
    LOADrequest* loadRequest;
    u16 logoSize = (u16)RSC_DISK1.metaData[ RSC_DISK1.FAT[RSC_DISK1_POLYZOOM__REBIRTH_ARJX].metadataIndex ].originalsize;


	IGNORE_PARAM(_fsm);

	packed = (u8*) RINGallocatorAlloc ( &sys.mem, LOADgetEntrySize(&RSC_DISK1, RSC_DISK1_POLYZOOM__REBIRTH_ARJX) );
