/*
**
** test program for testing the Ni packer depack code, depack data is forced tot even adress.
**
...
{ /* decode the data pointed to data */
	...
	if((((int)data)&1)==1)
	{ /* data is on odd adress, copy to even adress */
		memcpy(data-1, data, compressed_size);
		data--;
	}
	start = clock();
	switch(mode)
